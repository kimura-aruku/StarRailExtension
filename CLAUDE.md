...
- 拡張機能は戦績が「数値」モード（「簡略」モードではない）である必要があります（簡略モード→数値モード切り替え時の自動再描画対応済み）
- カスタムサブステータスパネルからハイライトされたサブステータスのみでスコアを計算します
- 多言語対応：日本語（JP）と英語（EN）に対応、言語切り替え時の自動再描画対応済み
- 注入される全ての要素は識別とクリーンアップのためにCSSクラス`alk-element`を使用
- 動的コンテンツの読み込みとユーザー操作を処理するためにmutation observerを使用
- パフォーマンス最適化：デバウンス機能と初期化状態管理により無駄な再描画を最小限に抑制
...
### パフォーマンス面の改善点
- **MutationObserver最適化**: ✅ **対応済み** - デバウンス機能（50ms）と初期化状態管理により無駄な再描画を削減
...
- **国際化対応**: ✅ **対応済み** - 日本語（JP）と英語（EN）に対応、言語切り替え時の自動再描画機能実装済み
